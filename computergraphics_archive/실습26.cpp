...
void main(int argc, char** argv) //--- 윈도우 출력하고 콜백함수 설정
{
    ...
    InitBuffer();
    InitAxisBuffer();
    LoadOBJ("n_sphere.obj", 0);
    LoadOBJ("n_sphere.obj", 1);
    LoadOBJ("n_sphere.obj", 2);
...
GLvoid Timer(int value) //--- 콜백 함수: 타이머 콜백 함수
{
    ...
    for (int i = 0; i < shapes.size(); i++) {
        ...
        if (shapes[i].object_num == 0) {
            shapes[i].model = glm::scale(shapes[i].model, glm::vec3(1.5f));
        }
        else if (shapes[i].object_num == 1) {
            shapes[i].model = glm::translate(shapes[i].model, glm::vec3(3.0f, 0.0f, 0.0f));
            shapes[i].model = glm::scale(shapes[i].model, glm::vec3(1.2f));
        }
        else if (shapes[i].object_num == 2) {
            shapes[i].model = glm::translate(shapes[i].model, glm::vec3(-3.0f, 0.0f, 0.0f));
            shapes[i].model = glm::scale(shapes[i].model, glm::vec3(1.0f));
        }
        else if (shapes[i].object_num == 3) {
            shapes[i].model = glm::rotate(shapes[i].model, glm::radians(r_radian_stack), glm::vec3(0.0f, 1.0f, 0.0f));
            shapes[i].model = glm::translate(shapes[i].model, glm::vec3(0.0f, 0.0f, light_x));
    ...
    light_model = glm::translate(light_model, glm::vec3(0.0f, 0.0f, light_x));
