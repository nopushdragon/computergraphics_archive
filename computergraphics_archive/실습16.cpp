        ...
        if (shapes[i].object_num == now_object) {
            for (int j = 0; j < now_face.size(); j++) {
                if (now_face[j] == shapes[i].face_count) {
                    if(wire_solid) glDrawArrays(GL_TRIANGLES, first, vertexCount);
                    else glDrawArrays(GL_LINE_LOOP, first, vertexCount);
                }
            }
        }
        first += vertexCount;
...
GLvoid Timer(int value) //--- 콜백 함수: 타이머 콜백 함수
{
    if (isx) {
        for (int i = 0;i < shapes.size();i++) {
            if (shapes[i].object_num == 0) // 큐브
                shapes[i].model = glm::rotate(shapes[i].model, glm::radians(1.0f), glm::vec3(1.0f, 0.0f, 0.0f));
            else if (shapes[i].object_num == 1) // 피라미드
                shapes[i].model = glm::rotate(shapes[i].model, glm::radians(1.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        }
    }
    else if (isX) {
        ...
        for (int i = 0;i < shapes.size();i++) {
            if (shapes[i].object_num == 0) // 큐브
                shapes[i].model = glm::rotate(shapes[i].model, glm::radians(1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
            else if (shapes[i].object_num == 1) // 피라미드
                shapes[i].model = glm::rotate(shapes[i].model, glm::radians(1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        }
    }
